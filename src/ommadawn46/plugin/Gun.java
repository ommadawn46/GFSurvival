...
public class Gun extends GFSItem{
	...
	private void shot(Player player){
		...
		if(ammoRemain > 0){
			...
			Projectile bullet = (Projectile) player.getWorld().spawnEntity(loc.add(loc.getDirection().getX()*1.5, 1.6 + loc.getDirection().getY()*1.5, loc.getDirection().getZ()*1.5), bulletType);
